...
export async function POST(request: Request, context: Params) {
  ...
  const parsed = applyOptimizationSchema.safeParse(body);

  if (!parsed.success) {
    return NextResponse.json(
      { success: false, message: parsed.error.issues[0]?.message ?? "No se pudo aplicar la propuesta." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { success: false, message: error instanceof Error ? error.message : "No se pudo aplicar la propuesta." },
      { status: 400 }
    );
  }

  return NextResponse.json({
    message: "Ruta optimizada aplicada.",
    success: true
  });
}
