...
export async function PATCH(request: Request, context: Params) {
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { success: false, message: parsed.error.issues[0]?.message ?? "No se pudo actualizar el depósito." },
      { status: 400 }
    );
  }
  ...
  if (error) {
    console.error("logistics depot update failed", error);
    return NextResponse.json(
      { success: false, message: "No se pudo actualizar el depósito." },
      { status: 500 }
    );
  }
