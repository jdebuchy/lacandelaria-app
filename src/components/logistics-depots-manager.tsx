...
export function LogisticsDepotsManager({ depots }: LogisticsDepotsManagerProps) {
  ...
  const inputClass =
    "h-11 rounded-control border border-line bg-paper-muted px-4 text-sm text-ink outline-hidden transition focus:border-info-line";

  return (
    <section className="grid gap-5">
      <div className="flex flex-col gap-2">
        <h2 className="text-lg font-semibold text-ink">Depósitos de salida</h2>
      ...
      {feedback.message ? (
        <p className={`mt-4 text-sm ${feedback.success ? "text-accent" : "text-danger-fg"}`}>
          {feedback.message}
        </p>
      ) : null}

      <form onSubmit={handleCreate} className="mt-5 grid gap-4 rounded-card border border-line bg-paper-muted p-4">
        <p className="text-sm font-medium text-ink">Nuevo depósito</p>
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="text-sm font-semibold text-ink">{depot.label}</p>
                      <span className="rounded-control border border-line bg-paper px-2.5 py-1 text-xs text-ink-soft">
                  ...
                  <button
                    ...
                    className="inline-flex h-10 items-center justify-center rounded-control border border-line px-4 text-sm font-medium text-ink transition hover:border-line-strong disabled:cursor-not-allowed disabled:opacity-60"
                  >
                ...
                <div className="mt-4 grid gap-4">
                  ...
                  <label className="grid gap-2 text-sm text-ink-soft">
                    ...
                    <input
                      ...
                      readOnly
                      className="h-10 rounded-control border border-line bg-paper-muted px-3 text-sm text-ink-faint outline-hidden"
                      placeholder="Se completa al elegir una sugerencia"
                    />
                  </label>
                </div>

                <label className="mt-3 flex items-center gap-3 text-sm text-ink-soft">
                  <input
                    type="checkbox"
                    checked={form.active}
